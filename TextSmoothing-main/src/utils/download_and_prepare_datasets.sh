...
# GossipCop dataset
mkdir -p datasets/gossip
for split in train dev test;
do
  cp "D:/360MoveData/Users/jingzheng/Desktop/大作业/TextSmoothing-main/src/data.tsv" datasets/gossip/${split}.raw
  python convert_num_to_text_labels.py -i datasets/gossip/${split}.raw -o datasets/gossip/${split}.tsv -d gossip
  rm datasets/gossip/${split}.raw
done
python create_fsl_dataset.py -datadir datasets/gossip -num_train 10 -num_dev $NUMDEV -sim $NUMEXP -lower