...
CACHE=/d/360MoveData/Users/jingzheng/Desktop/大作业/CACHE
PREFIXSIZE=3
MAXEPOCH=30
TASK=snips
temp_rate=1.0
         ...
         for mr in 40;
              do
                ...
                for MASKLEN in span word;
                    ...
                    python $SRC/bert_aug/bert_classifier.py --task $TASK --data_dir $MODELDIR --seed ${i} --cache $CACHE  --temp_rate ${temp_rate} 
                    --smooth_rate ${smooth_rate}  > $RAWDATADIR/bert_bart_l5_${MASKLEN}_mask_${MRATIO}_prefix_${PREFIXSIZE}.log
                done
           done
    done
done
