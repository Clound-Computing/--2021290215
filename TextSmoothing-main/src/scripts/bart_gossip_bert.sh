...
for NUMEXAMPLES in 10; do
    for i in {0..14}; do
        ...
        fairseq-preprocess --user-dir=$SRC/bart_aug --only-source \
                    ...
                    --testpref $RAWDATADIR/test_bpe.src \
                    --destdir $DATABIN \
                    --srcdict $BART_PATH/dict.txt

        for mr in 40; do
            MRATIO=0.${mr}
            for MASKLEN in word span; do
                ...
                CUDA_VISIBLE_DEVICES=0 fairseq-train $DATABIN/ \
                    ...
                    --task mask_s2s \
                    --bpe gpt2 \
                    --gpt2_encoder-json $SRC/utils/gpt2_bpe/encoder.json \
                    --gpt2_vocab-bpe $SRC/utils/gpt2_bpe/vocab.bpe \
                    --layernorm-embedding \
